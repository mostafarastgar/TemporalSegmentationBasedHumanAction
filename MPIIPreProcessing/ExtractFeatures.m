labels = load('../data/MPII/labels.mat');
labels = labels.labels;
features = zeros(300000, 771);
index = 1;
firstJ = 1;
for(i=1:size(labels, 1))
    dir_perfix = strcat('../data/MPII/clips/', num2str(i), '/');
    ...
    for(j=firstJ:size(file_names,2))
        ...
        if(index+size(stips, 1)-1>300000)
            features(index:end, :) = [];
            save(strcat('../data/MPII/features/features_',num2str(i), '_', num2str(j),'.mat'), 'features', '-v7.3');
            features = zeros(300000, 771);
            index = 1;
        end
...
features(index:end, :) = [];
save('../data/MPII/features/features.mat', 'features', '-v7.3');